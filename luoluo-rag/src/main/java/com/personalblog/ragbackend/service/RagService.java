...
public class RagService {
    ...
    public RagService(RagApiClient ragApiClient, PostRepository postRepository, RagProperties ragProperties) {
    ...
    private List<RetrievedChunk> localRetrieve(String question) {
        ...
        return postRepository.findAll().stream()
                ...
                .limit(ragProperties.getRag().getTopK())
                .toList();
    }
    ...
    private String localAnswer(List<RetrievedChunk> chunks) {
        if (chunks.isEmpty()) {
            return "No relevant local blog context was found.";
        }
        String titles = chunks.stream().map(RetrievedChunk::title).reduce((a, b) -> a + ", " + b).orElse("");
        return "Relevant context was found in: " + titles + ".";
    }
}
