...
-- Version: V20260319
-- Module: member
-- Description: Create member auth tables
-- Date: 2026-03-19
-- ============================================================
...
create table if not exists member_user (
    ...
    deleted         tinyint not null default 0 comment 'Logical delete flag: 0 no, 1 yes',
    created_at      datetime not null default current_timestamp comment 'Created time',
    updated_at      datetime not null default current_timestamp on update current_timestamp comment 'Updated time',
    primary key (id),
    unique key uk_member_user_username_deleted (username, deleted),
    unique key uk_member_user_phone_deleted (phone, deleted),
    unique key uk_member_user_email_deleted (email, deleted),
    key idx_member_user_status_deleted (status, deleted)
...
create table if not exists member_verify_code (
    ...
    created_at      datetime not null default current_timestamp comment 'Created time',
    primary key (id),
    key idx_member_verify_code_target (target_type, target_value, used, deleted)
...
create table if not exists member_login_session (
    ...
    created_at      datetime not null default current_timestamp comment 'Created time',
    primary key (id),
    unique key uk_member_login_session_token_deleted (token, deleted),
    key idx_member_login_session_user_id_deleted (user_id, deleted)
