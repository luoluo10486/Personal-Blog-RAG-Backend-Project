...
@Component
public class SearchKnowledgeBaseMcpExecutor {

    private static final String TOOL_ID = "searchKnowledgeBase";

    private final RagMcpTools ragMcpTools;

    public SearchKnowledgeBaseMcpExecutor(RagMcpTools ragMcpTools) {
        this.ragMcpTools = ragMcpTools;
    }

    @Bean
    public McpServerFeatures.SyncToolSpecification searchKnowledgeBaseToolSpecification() {
    ...
    private Tool buildTool() {
        ...
        properties.put("query", Map.of(
                "type", "string",
                "description", "检索问题，例如：会员积分可以提现吗"
        ));
        properties.put("topK", Map.of(
                "type", "integer",
                "description", "返回结果数量，范围 1 到 20，默认 5"
        ));
        properties.put("baseCode", Map.of(
                "type", "string",
                "description", "可选知识库编码，留空时使用默认知识库"
        ));

        JsonSchema inputSchema = new JsonSchema("object", properties, List.of("query"), null, null, null);
        return Tool.builder()
                .name(TOOL_ID)
                .description("在知识库中执行检索，返回命中的知识片段和向量空间信息")
    ...
    private CallToolResult handleCall(CallToolRequest request) {
        ...
        try {
            ...
            String result = ragMcpTools.searchKnowledgeBase(
